...
class ChangellyOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self, trading_pairs: Optional[List[str]] = None,):
        super().__init__(ChangellyAPIOrderBookDataSource(trading_pairs), trading_pairs)
        ...
        self._order_books: Dict[str, ChangellyOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[ChangellyOrderBookMessage]] = \
            defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, ChangellyActiveOrderTracker] = defaultdict(ChangellyActiveOrderTracker)
    ...
    async def _track_single_book(self, trading_pair: str):
        ...
        """
        past_diffs_window: Deque[ChangellyOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: ChangellyOrderBook = self._order_books[trading_pair]
        active_order_tracker: ChangellyActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: ChangellyOrderBookMessage = None
                saved_messages: Deque[ChangellyOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[ChangellyOrderBookMessage] = list(past_diffs_window)
