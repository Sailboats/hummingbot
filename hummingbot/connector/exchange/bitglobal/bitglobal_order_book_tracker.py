...
class BitglobalOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
                 ...
                 domain: str = "com"):
        super().__init__(
            data_source=BitglobalAPIOrderBookDataSource(trading_pairs=trading_pairs, domain=domain),
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ...
                # self.logger().debug(f'ob_message = {ob_message}')
                trading_pair: str = ob_message.trading_pair
                # self.logger().debug(f'trading_pair = {trading_pair}')

                if trading_pair not in self._tracking_message_queues:
                    # self.logger().debug(f'trading_pair not in self._tracking_message_queues')
                    messages_queued += 1
                ...
                # self.logger().debug(f'message_queue = {message_queue}')
                # Check the order book's initial update ID. If it's larger, don't bother.
                order_book: OrderBook = self._order_books[trading_pair]
                # self.logger().debug(f'order_book = {order_book}')
                if order_book.snapshot_uid > ob_message.update_id:
                    # self.logger().debug(f'order_book.snapshot_uid > ob_message.update_id')
                    messages_rejected += 1
                    continue
