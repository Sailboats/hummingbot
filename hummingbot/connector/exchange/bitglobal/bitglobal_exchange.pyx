...
from hummingbot.connector.exchange.bitglobal.bitglobal_api_order_book_data_source import BitglobalAPIOrderBookDataSource
from hummingbot.connector.exchange.bitglobal.bitglobal_auth import BitglobalAuth
from hummingbot.connector.exchange.bitglobal.bitglobal_in_flight_order import BitglobalInFlightOrder
from hummingbot.connector.exchange.bitglobal.bitglobal_order_book_tracker import BitglobalOrderBookTracker
from hummingbot.connector.trading_rule cimport TradingRule
from hummingbot.connector.exchange_base import (
    ExchangeBase,
    s_decimal_NaN)
from hummingbot.connector.exchange.bitglobal.bitglobal_user_stream_tracker import BitglobalUserStreamTracker
from hummingbot.core.utils.tracking_nonce import get_tracking_nonce
from hummingbot.core.utils.estimate_fee import estimate_fee

from hummingbot.connector.exchange.bitglobal.bitglobal_constants import *
...
class BitglobalAPIError(IOError):
    def __init__(self, error_payload: Dict[str, Any]):
        super().__init__(str(error_payload))
        self.error_payload = error_payload


cdef class BitglobalExchangeTransactionTracker(TransactionTracker):
    cdef:
        BitglobalExchange _owner

    def __init__(self, owner: BitglobalExchange):
        super().__init__()
        self._owner = owner
...
cdef class BitglobalExchange(ExchangeBase):
    ...
    def __init__(self,
                 bitglobal_api_key: str,
                 bitglobal_api_secret: str,
        ...
        self._bitglobal_auth = BitglobalAuth(api_key=bitglobal_api_key, secret_key=bitglobal_api_secret)
        self._in_flight_orders = {}
        self._last_poll_timestamp = 0
        self._last_timestamp = 0
        self._order_book_tracker = BitglobalOrderBookTracker(
        ...
        self._tx_tracker = BitglobalExchangeTransactionTracker(self)
        self._user_stream_tracker = BitglobalUserStreamTracker(bitglobal_auth=self._bitglobal_auth,
                                                          trading_pairs=trading_pairs)
    ...
    @property
    def order_book_tracker(self) -> BitglobalOrderBookTracker:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BitglobalInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        self._in_flight_orders.update({
            key: BitglobalInFlightOrder.from_json(value)
    ...
    @property
    def user_stream_tracker(self) -> BitglobalUserStreamTracker:
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await BitglobalAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def check_network(self) -> NetworkStatus:
        try:
            await self._api_request(method="GET", path_url=SERVER_TIME)
    ...
    async def _api_request(self,
        ...
        public_params = {
            'apiKey': self._bitglobal_auth.api_key,
            # 'bizCode': action,
            'msgNo': ts,
            'timestamp': ts,
        }
        ...
        url = BASE_ENDPOINT + path_url
        client = await self._http_client()

        if is_auth_required:
#            headers.update(self._bitglobal_auth.generate_rest_auth(data))
            data["signature"] = self._bitglobal_auth.generate_rest_auth(data)
    ...
    async def _update_balances(self):
        ...
        params = {'coinType': '', 'assetType': 'spot'}
        msg = await self._api_request("POST", path_url=path_url, data=params, is_auth_required=True)
    ...
    cdef object c_get_fee(self,
        ...
        return estimate_fee("bitglobal", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self._api_request("GET", path_url=INSTRUMENTS_URL)
            trading_rules_list = self._format_trading_rules(exchange_info["data"]["coinConfig"], exchange_info["data"]["spotConfig"])
            self._trading_rules.clear()
            for trading_rule in trading_rules_list:
                self._trading_rules[trading_rule.trading_pair] = trading_rule

    def _format_trading_rules(self, coinConfig: List[Dict[str, Any]], spotConfig: List[Dict[str, Any]]) -> List[TradingRule]:
        cdef:
            list trading_rules = []

        for info in spotConfig:
            try:
...
#                                min_base_amount_increment=Decimal(info["multiplierUp"]),
    ...
    async def get_order_status(self, exchange_order_id: str, trading_pair: str) -> Dict[str, Any]:
        params = {'orderId': exchange_order_id, 'symbol': trading_pair}
        msg = await self._api_request("POST",
                                      path_url=ORDER_DETAILS_URL,
                                      data=params,
    ...
    async def _update_order_status(self):
        ...
        for tracked_order in tracked_orders:
            ...
            if execute_amount_diff > s_decimal_0:
                execute_price = Decimal(order_update["avgPrice"])
                tracked_order.executed_amount_base = new_confirmed_amount
                tracked_order.executed_amount_quote = new_confirmed_amount * execute_price
                tracked_order.fee_paid = self.get_fee(tracked_order.base_asset,
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Bitglobal. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Bitglobal. "
    ...
    async def _user_stream_event_listener(self):
        async for stream_message in self._iter_user_stream_queue():
            try:
                code = stream_message.get("code", None)
                if code not in BITGLOBAL_WS_CODES:
                    continue
                if "data" not in stream_message:
                    continue

                # stream_message["data"] is a Dict
                # only ORDER topic for bitglobal
...
#                if channel == OKEX_WS_CHANNEL_ORDERS:
                order_id = data["oId"]
                client_order_id = data["oId"]
                trading_pair = data["symbol"]
                order_status = data["status"]

                if order_status not in ("created", "partDealt", "fullDealt", "canceled"):
                ...
                execute_amount_diff = Decimal(data["dealQuantity"])

                if execute_amount_diff > s_decimal_0:
                    execute_price = Decimal(data["dealPrice"])
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Bitglobal for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price if order_type is OrderType.LIMIT else '-1'}."
                f"{decimal_price}.",
                exc_info=True,
                app_warning_msg="Failed to submit buy order to Bitglobal. Check API key and network connection."
            )
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Bitglobal. "
        ...
        for trading_pair in orders_by_trading_pair:
            ...
            try:
                cancel_all_results = await self._api_request(
                    "POST",
                    path_url=BATCH_ORDER_CANCEL,
                    data=data,
                    is_auth_required=True
                )

                if cancel_all_results['code']=='0':
                    for order_id in cancel_order_ids:
                        cancellation_results.append(CancellationResult(order_id, '0'))
                        self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
                                             OrderCancelledEvent(self._current_timestamp,
                                                                 order_id,
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Failed to cancel all orders on Bitglobal. Check API key and network connection."
                )
